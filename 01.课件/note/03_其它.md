...
- 2XX: 表示成功处理请求
  ...
  - 201,参数格式不对
- 3XX: 需要重定向, 浏览器直接跳转
  - 302->当用户请求A链接,结果被A服务器引导到了B链接上,地址重定向
  - 304->使用缓存,当第二个请求服务器的某个资源时,服务器发现该资源在浏览器上有存储,那么不返回文件内容,直接返回响应,响应码为304,浏览器知道后,会自己去内存中找到对应的文件
- 4XX: 客户端请求错误
  - 403->服务器上有该资源,但是拒绝给你(意思是权限不足)
  - 404->在服务器上找不到该资源(意思就是没有你要的资源,大概率是前端路径错误)
- 5XX: 服务器端错误
  - 500->服务器崩溃,无法处理当前代码
  - 501->服务器无法处理当前请求,请求方式不对
...
1. 配置通用的基础路径和超时时间
   1. 为什么要配置基础路径
      1. 最主要的目的就是给每个请求添加上一个相同的基础路径
      2. 实际上是因为proxy代理模式需要有一个前缀,告知开发服务器,哪些请求需要转发
   2. 为什么要配置超时时间
      1. 如果一个请求超过了最大超时时间还没有响应,那么当前请求就会失败,变为404
      2. 因为浏览器具有最大请求并发数限制,小程序最多是10个,Chrome和Firefox最多是6个
2. 显示请求进度条
   1. 显示进度条: 请求拦截器回调
   2. 结束进度条: 响应拦截器回调
...
- cookie
  - 有max-age属性,超过最大存活时间,数据就会丢失
  - 没有max-age属性,没写该属性就是session cookie,只要关闭当前标签页或者浏览器,数据就会丢失
- sessionStorage
  - 关闭当前标签页或者浏览器,数据就会丢失
  - 刷新当前标签页,数据不会丢失
  - 复制当前标签页,数据不会丢失,会复制一份全新的
  - 复制当前网址,打开新的标签页,输入该网址,新标签页不会有旧的sessionStorage的数据
- localStorage
  - 只要开发者不删除数据,那么数据将永久存在
...
- 可用document.cookie读取或保存
  - cookie的使用会受到服务器的控制,服务器可以通过httpOnly属性控制cookie的可见性
  - 用户可以禁用cookie
- 可以利用cookies工具库简化编码
...
​	**统一称呼,需要传递数据的叫做A页面,需要接收数据的叫做B页面**

1. **使用localStorage实现跨标签页通信(两个标签页域名相同)**
   1. **B页面给window对象绑定事件监听,事件名是storage,用于监视当前域名下localStorage的设置/修改**
   2. **A页面使用setItem语法,将数据存入localStorage中,就会触发B页面的storage事件**
   3. **在事件的回调函数中,可以获取到修改的key名称以及最新属性值**
2. **通过跳转时候,将参数拼接在url中(只能发生在页面跳转的一瞬间)**
   1. **A页面,使用window.location.href=url,打开一个新的网页,在路径中拼接参数即可**
   2. **B页面,在加载显示的时候,js代码自动读取当前路径中的数据即可**
3. **使用domain属性实现(可以实现两个不同域名之间传递数据,但是这两个域名必须具有共同的上级域名)**
   1. **B页面将自己的document.domain属性设置为上级域名**
   2. **B页面使用window.open方法,打开A页面,同时open方法会返回A页面的window对象**
   3. **A页面被打开之后,修改自己的document.domain属性为上级域名**
   4. **A页面向widnow对象身上添加需要传递的数据**
   5. **B页面通过流程2中获取到的window对象,可以读取到A页面的数据**

### 从输入url到渲染出页面的整个过程

1. DNS 解析(用处:将域名解析成对应的服务器IP地址)
   1. 域名的作用
      1. 主要目的就是因为方便记忆,方便推广
      2. 其实域名这东西反而降低了页面的展示速度
...
- layout 布局：计算元素的位置和大小信息
  - 重排(重新布局):当页面上的某个元素发生位置的变化或者大小的变化,那么当前页面就必定会发生重排操作
- render 渲染：将颜色/文字/图片等渲染上去
  - 重绘(重新渲染):当页面上的某个元素的颜色等信息发生变化,那么当前页面就必定会发生重绘操作
    - 也就是说,想要看到最新的页面效果,一定会经过重绘操作
    - 重排必定伴随着重绘,重绘不一定伴随着重排
